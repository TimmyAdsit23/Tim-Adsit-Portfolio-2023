...
if [ ! -f Garland.java ]; then
	echo ""
	echo " Error: You must place Garland.java in this directory before we can"
...
do
	...
	# Attempt to compile.
	javac Garland.java TestCase$i.java 2> /dev/null
	compile_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " Garland.java | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" Garland.java | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" Garland.java | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" Garland.java -n"
		echo "     grep \"\$(printf '^\t')\" Garland.java -n"
		echo ""
		echo "  --------------------------------------------------------------"
...
if [ $PASS_CNT -eq $TOTAL_TEST_CNT ]; then
	...
	echo "  see Section 5 of the assignment PDF (pgs. 8-9)."
	echo ""

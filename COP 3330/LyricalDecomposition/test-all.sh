...
# =================================
# LyricalDecomposition: test-all.sh
# =================================
...
if [ ! -f LyricalDecomposition.java ]; then
	echo ""
	echo " Error: You must place LyricalDecomposition.java in this directory before we can"
...
grep -s -q "^import " LyricalDecomposition.java
grep_val=$?

if [ "$grep_val" -eq "0" ]; then
	echo ""
	echo "  Whoa, buddy! Your source file appears to have at least one import"
	echo "  statement. For this particular assignment, import statements are not"
	echo "  allowed. Please remove all import statements in order to proceed."
	echo ""
	exit
fi
...
do
	...
	# Attempt to compile.
	javac LyricalDecomposition.java TestCase$i.java 2> /dev/null
	compile_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " LyricalDecomposition.java | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" LyricalDecomposition.java | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" LyricalDecomposition.java | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" LyricalDecomposition.java -n"
		echo "     grep \"\$(printf '^\t')\" LyricalDecomposition.java -n"
		echo ""
		echo "  --------------------------------------------------------------"

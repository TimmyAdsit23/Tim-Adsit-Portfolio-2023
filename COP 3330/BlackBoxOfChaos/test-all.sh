...
if [ ! -f BlackBoxOfChaos.java ]; then
	echo ""
	echo " Error: You must place BlackBoxOfChaos.java in this directory before we can"
...
do
	...
	# Attempt to compile.
	javac BlackBoxOfChaos.java TestCase$i.java 2> /dev/null
	compile_val=$?
	...
	java TestCase$i &> myoutput.txt
	execution_val=$?
	if [ "$execution_val" != 0 ] && [ "$i" != "06" ]; then
...
javac BlackBoxOfChaos.java 2> BlackBoxOfChaos__warning.log

grep unchecked BlackBoxOfChaos__warning.log > /dev/null 2> /dev/null
warn_val=$?

grep "@Suppress" BlackBoxOfChaos.java > /dev/null 2> /dev/null
supp_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " BlackBoxOfChaos.java | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" BlackBoxOfChaos.java | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" BlackBoxOfChaos.java | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" BlackBoxOfChaos.java -n"
		echo "     grep \"\$(printf '^\t')\" BlackBoxOfChaos.java -n"
		echo ""
		echo "  --------------------------------------------------------------"
...
rm -f *.class
rm -f BlackBoxOfChaos__warning.log
...
if [ $PASS_CNT -eq $TOTAL_TEST_CNT ]; then
	...
	echo "  see Section 4 of the assignment PDF (pgs. 3-4)."
	echo ""
	...
	if [[ $warn_note != 0 ]]; then
		echo ""
		echo "  You can type 'javac BlackBoxOfChaos.java -Xlint:unchecked' at the"

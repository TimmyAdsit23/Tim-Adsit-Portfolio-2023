...
grep -s -q "^#include \"KindredSpirits\.h\"" KindredSpirits.c
grep_val=$?

if [[ $grep_val != 0 ]]; then
	echo ""
	echo "  Whoa, buddy! Your KindredSpirits.c file does not appear to have"
	echo "  a proper #include statement for KindredSpirits.h. Please read"
	echo "  Section 3 on pg. 5 of the assignment PDF before proceeding."
	echo ""
	exit
fi
...
if ! [ -x "$(command -v grep)" ]; then
	...
	gcc KindredSpirits.c -c &> ./err.log
	compile_flag=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " KindredSpirits.c | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" KindredSpirits.c | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" KindredSpirits.c | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" KindredSpirits.c -n"
		echo "     grep \"\$(printf '^\t')\" KindredSpirits.c -n"
		echo ""
		echo "  --------------------------------------------------------------"
...
if [ $PASS_CNT -eq $TOTAL_TEST_CNT ]; then
	...
	echo "  see Section 7 of the assignment PDF (pg. 7-8)."
	echo ""

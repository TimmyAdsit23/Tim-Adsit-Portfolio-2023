...
// ===========================
// StringMuncher: testcase25.c
// ===========================
// Tests gobbleGulp() functionality. In this one, gobbleGulp() should trigger
// an expansion.
...
int main(void)
{
	...
	// Trigger an expansion by placing dividers between existing strings.
	gobbleGulp(munchy, 1, "-----------");
	gobbleGulp(munchy, 4, "------");
	gobbleGulp(munchy, 7, "------");
	gobbleGulp(munchy, 10, "---------");

	printf("\n");
	printMunchyWithHeader(munchy, "After final gulps:",  munchy->num_stomachs_occupied);

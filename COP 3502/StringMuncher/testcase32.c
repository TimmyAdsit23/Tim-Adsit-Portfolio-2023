...
// ===========================
// StringMuncher: testcase32.c
// ===========================
// Further tests of evacuate() functionality.
...
int main(void)
{
	...
	assert(evacuate(munchy) == 1);

	printf("\n");
	printf("After first evacuate:\n");
	printf("=====================\n");
	printStomachContents(munchy);

	// These should be no-ops since munchy is empty and since NULL is, well, NULL.
	assert(evacuate(munchy) == 0);
	assert(evacuate(NULL) == 0);

	printf("\n");
	printf("After no-op evacuations:\n");
	printf("========================\n");
	printStomachContents(munchy);

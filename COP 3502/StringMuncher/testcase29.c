...
// ===========================
// StringMuncher: testcase29.c
// ===========================
// Further tests of heave() functionality.
...
int main(void)
{
	...
	assert(heave(munchy) == 1);

	printf("\n");
	printf("After first heave:\n");
	printf("==================\n");
	printStomachContents(munchy);

	// These should be no-ops since munchy is empty and since NULL is, well, NULL.
	assert(heave(munchy) == 0);
	assert(heave(NULL) == 0);

	printf("\n");
	printf("After dry heaves (sorry!):\n");
	printf("==========================\n");
	printStomachContents(munchy);

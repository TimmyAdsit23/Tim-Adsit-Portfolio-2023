...
// ===========================
// StringMuncher: testcase33.c
// ===========================
// Tests digest() functionality.
//
// Also checks that contractDigestiveTract() uses "stomach" (singular) in its
// output when we contract down to just one stomach.
//
// This test is similar to testcase20.c, but without the subsequent expansions,
// and with the addition of the digest() function call.
...
int main(void)
{
	...
	checkInternals(munchy, name, num_stomachs = 1, num_stomachs_occupied = 0, num_snacks, snacks);
